...
@Slf4j
public class JpaTableMappingFactory extends AbstractTableMappingFactory {
    ...
    @Override
    protected boolean isMarkedId(FieldMapping field) {
        Id id = getAnnotation(field, Id.class);
        return id != null;
    }

    @Override
    protected String getReferencedColumnName(FieldMapping field) {
    ...
    @Override
    protected String getJoinColumnName(FieldMapping field) {
        JoinColumn annotation = getAnnotation(field, JoinColumn.class);
    ...
    @Override
    protected boolean isVersionField(FieldMapping field) {
    ...
    @Override
    protected boolean isTransient(FieldMapping field) {
    ...
    @Override
    protected boolean isBasicField(FieldMapping field) {
    ...
    protected String getColumnName(FieldMapping field) {
        Column column = getAnnotation(field, Column.class);
        String columnName;
        if (column != null && !column.name().isEmpty()) {
            columnName = column.name();
        } else {
            columnName = field.fieldName().replaceAll("([a-z])([A-Z])", "$1_$2").toLowerCase();
        }
